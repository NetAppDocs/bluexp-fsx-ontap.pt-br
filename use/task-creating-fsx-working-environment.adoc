...
* O ARN de uma função do IAM que dá ao Workload Factory as permissões necessárias para criar um sistema de arquivos FSX for ONTAP. link:task-setting-up-permissions-fsx.html["Saiba como conceder permissões a uma conta da AWS"^].
